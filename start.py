...
async def main() -> None:
    ...
    # ── 1. DB + EventBus ──
    from backend.db.models import init_db, get_async_db, fetch_one, insert, now_iso
    ...
    async def _on_agent_output(agent_name: str, text: str, usage=None):
        ...
        # 記錄 session
        import uuid
        from backend.db.models import get_async_db as _get_db, insert as _insert, now_iso as _now
        conn = await _get_db()
        try:
            # 用實際 token 數計算 cost
            if usage:
